...
# 데이터베이스 엔진을 생성한다.
# engine = create_engine('postgresql://test:password@localhost:5432/test_db', echo=True)
...
def visit_create_table_with_serial_support(
    ...
    seqs = list(set(seqs)) # 종복제거
    seq_sql = ""
    for seq in seqs: seq_sql += f"\nCREATE SEQUENCE {seq};"
...
# def _make_cloned_table(table_type: type[DeclarativeMeta]) -> Table:
#     table = table_type.__table__
#     cols: list[Column] = []
#     for col in table.columns:
#         if table.autoincrement_column == col and type(col.default) is not Sequence:
...
MyTestTable.__table__.create(bind=engine)

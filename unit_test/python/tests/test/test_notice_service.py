...
from genai.system_admin.adapter.outbound.persistence.system_notice_repository import SystemNoticeRepository
from genai.system_admin.application.condition import NoticeSearchConditionParams
from genai.system_admin.application.service.system_notice_service import SystemNoticeService
...
class TestNoticeService:
    ...
    def mock_repository(self):
        mock_repo = Mock(spec=SystemNoticeRepository)
    ...
    @pytest.fixture
    def notice_service(self, mock_repository):
        return SystemNoticeService(repository=mock_repository)

    def test_create_notice(self, notice_service):
        new_notice = NoticeDomain(
            notice_id=2,
            notice_title="New Notice",
            notice_contents="This is a new notice.",
            begin_date=datetime.now(UTC) - timedelta(days=2),
    ...
    def test_update_notice(self, notice_service):
        ...
        updated_notice = NoticeDomain(
            ...
            begin_date=datetime.now(UTC) - timedelta(days=3),
    ...
    def test_get_list(self, notice_service):
        pagination = PaginationParams(page=1, limit=10)
        condition = NoticeSearchConditionParams()
        result = notice_service.get_list(condition=condition, pagination=pagination)

        # Verify that the get_notice_page_list method was called with the correct arguments
        notice_service._repository.get_notice_page_list.assert_called_once_with(pagination, condition)

        # Check that the returned value contains the sample notice
        assert len(result) == 1
        assert result[0].notice_id == sample_notice.notice_id

    def test_get_list_without_pagination(self, notice_service):
        condition = NoticeSearchConditionParams()
        result = notice_service.get_list(condition=condition)

        # Verify that the get_notice_page_list method was called with the correct arguments
        notice_service._repository.get_notice_page_list.assert_called_once_with(condition)

        # Check that the returned value contains the sample notice
        assert len(result) == 1
        assert result[0].notice_id == sample_notice.notice_id

    def test_get_by_id(self, notice_service):
        result = notice_service.get_by_id(1)

        # Verify that the select_by_id method was called with the correct arguments
        notice_service._repository.select_by_id.assert_called_once_with(1)

        # Check that the returned value matches the sample notice
        assert result.notice_id == sample_notice.notice_id

    def test_delete_notice_successfully(self, notice_service):
        # Mock the repository's delete method to return the length of the list
        # notice_service._repository.delete.side_effect = lambda notice_id: sample_notice

        result = notice_service.delete(1)
    ...
    def test_delete_nonexistent_notice(self, notice_service):
        # Mock the repository's delete method to raise an exception
        notice_service._repository.delete.side_effect = ServerException.of(
            "The Notice [999] does not exist", HTTPStatus.NOT_FOUND
        )

        with pytest.raises(ServerException) as exc_info:
            notice_service.delete(999)
